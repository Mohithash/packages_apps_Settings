...
class VoltageVersionPreference :
    ...
    override fun getSummary(context: Context): CharSequence =
        SystemProperties.get(VOLTAGE_VERSION_PROPERTY, context.getString(R.string.unknown))
        ...
        if (userManager?.hasUserRestriction(UserManager.DISALLOW_FUN) == true) {
            ...
            val enforcedAdmin =
                RestrictedLockUtilsInternal.checkIfRestrictionEnforced(
                    context, UserManager.DISALLOW_FUN, myUserId)
            val disallowedBySystem =
                RestrictedLockUtilsInternal.hasBaseUserRestriction(
                    context, UserManager.DISALLOW_FUN, myUserId)
            if (enforcedAdmin != null && !disallowedBySystem) {
    ...
    companion object {
        ...
        const val VOLTAGE_VERSION_PROPERTY: String = "ro.bestrom.version"
